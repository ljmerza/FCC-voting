...
module.exports = function (passport) {
    ...
    // get user data from db by cookie session - try local then twitter then google
    passport.deserializeUser(function (user, done) {
        db.getLocalUser(user, function (err, user) {
            // if db error
            if (err) done(err)
    ...
    passport.use('local-login', new LocalStrategy({
        ...
        function (req, email, password, done) {
            ...
            process.nextTick(function () {
                ...
                db.getLocalUser( { email: email }, function (err, user) {
        ...
        function (req, email, password, done) {
            process.nextTick(function () {
                ...
                db.getLocalUser({ email: email }, function (err, user) {
                    ...
                    if (user) { 
                        return done(null, false, { error: 'Email is already taken.'} ) 
                    }
                    ...
                    if (email) {
                        ...
                        db.setLocalUser(user, function(err, userID) {
                            if (err) return done(err)
                            // set user id from db and pass back to route
                            user.userid = userID
                            return done(null, user)
                        })
                    }
